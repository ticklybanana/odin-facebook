class User < ApplicationRecord
  ...
  has_many :active_friends, -> { where friend_requests: { accepted: true } }, 
                           class_name: 'User',
                           through: :requests_sent,
                           source: :requested

  has_many :passive_friends, -> { where friend_requests: { accepted: true } }, 
                           class_name: 'User',
                           through: :requests_received, 
                           source: :requester

  def friends
    active_friends.union(passive_friends)
  end
end
